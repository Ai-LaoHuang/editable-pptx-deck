---
name: ai-laohuang-ppt-studio
description: >
  Ai-LaoHuang branded Chinese commercial PPT production skill. Use when the user
  asks to create, generate, redesign, or commercialize PPT/PPTX decks, especially
  Chinese marketing plans, business proposals, course decks, knowledge-IP decks,
  product introductions, 招商/路演/汇报 decks, or asks for "老黄PPT",
  "Ai-LaoHuang PPT Studio", "生成PPT", "做PPT", "制作演示文稿".
  Builds editable PPTX from source documents, topic briefs, SVG pages, native
  shapes, templates, and optional AI-generated images.
---

# Ai-LaoHuang PPT Studio Skill

> A branded, Chinese-first commercial PPT production workflow for generating editable PowerPoint decks from topics, documents, images, and structured design plans.

**Core Pipeline**: `Brief / Source Document -> Project -> Style Strategy -> Codex Image Assets -> SVG / Editable Structure -> Quality Check -> PPTX Export -> Preview`

## Ai-LaoHuang Positioning

This skill is a branded derivative of `hugohe3/ppt-master` under the MIT License.
Keep `LICENSE` and `NOTICE` with every redistributed copy. Do not remove upstream
copyright notices from copied code, templates, scripts, or substantial portions of
the original distribution.

Default product direction:

- Chinese business decks: marketing plans, product introductions, roadshows, course decks, consulting reports, 招商方案, 运营方案, 知识 IP 课件.
- Mixed visual strategy: use AI-generated bitmap images for cover / atmosphere / product scenes, and keep titles, diagrams, cards, metrics, labels, arrows, and charts as editable PPT-native objects whenever possible.
- Commercial polish: avoid screenshot-only slides; deliver an editable `.pptx`, preview images, and a short QA summary.
- Brand voice: concise, sharp, commercially useful, visually confident, and suitable for paid client delivery.

When the user gives only a topic, create a compact but complete deck brief first:
audience, objective, page count, visual direction, page outline, required images,
and expected deliverable path. If the topic depends on current facts, research it
before drafting.

When choosing between this skill's internal `image_gen.py` and Codex image
generation, prefer Codex image generation unless the user explicitly configured
and requested a provider in `.env`. Insert generated images as assets, not as
full-page screenshots, unless the user explicitly asks for image-only slides.
...
>
> - `ai-laohuang-ppt-studio` is a branded PPT production workflow, not a general application scaffold
